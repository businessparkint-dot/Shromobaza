  ...
  try {
    ...
    if (!worker) {
      return NextResponse.json(
        {
          error:
            "এই account-এর Worker profile পাওয়া যায়নি।",
        },
        { status: 404 }
      );
    }

    // Supabase nested relation sometimes returns an object
    // and sometimes an array depending on the relationship.
    ...
    if (applicationsError) {
      ...
      return NextResponse.json(
        {
          error:
            "Worker applications লোড করা যায়নি।",
