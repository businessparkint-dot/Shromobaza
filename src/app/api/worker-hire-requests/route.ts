...
export async function GET(request: NextRequest) {
  try {
    ...
    if (employerProfileIds.length > 0) {
      ...
      if (error) {
        console.error(
          "Employer profiles lookup error:",
          error
        );
      } else {
        profiles = data || [];
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // ---------------------------------------------------------
    // 1. Verify logged-in user is the Worker
    // ---------------------------------------------------------
    ...
    if (employer?.profile_id) {
      ...
      const notificationMessage =
        status === "accepted"
          ? `আপনার "${jobTitle}" Hire Request Worker গ্রহণ করেছেন।`
          : `আপনার "${jobTitle}" Hire Request Worker প্রত্যাখ্যান করেছেন।`;

      const { error: notificationError } =
        await supabaseAdmin
          .from("notifications")
          .insert({
            user_id: employer.profile_id,
            type: "job",
      ...
      if (notificationError) {
        console.error(
          "Hire Request notification error:",
          notificationError
        );
      } else {
        console.log(
          "Hire Request notification created:",
          employer.profile_id
        );
      }
    } else {
      console.warn(
        "Employer profile_id পাওয়া যায়নি। Notification তৈরি করা হয়নি."
      );
    }
    ...
    return NextResponse.json({
      ...
      message:
        status === "accepted"
          ? "Hire Request গ্রহণ করা হয়েছে। Employer-কে notification পাঠানো হয়েছে।"
          : "Hire Request প্রত্যাখ্যান করা হয়েছে। Employer-কে notification পাঠানো হয়েছে।",
    });
  } catch (error) {
    console.error(
