...
if (userError || !user) {
  return NextResponse.json(
    {
      error:
        "আপনার Login session বৈধ নয়। আবার Login করুন।",
    },
    { status: 401 }
  );
}
...
if (job.status && job.status !== "open") {
  return NextResponse.json(
    {
      error:
        "এই Job বর্তমানে আবেদন নেওয়া হচ্ছে না।",
    },
    { status: 400 }
  );
}
...
if (workerError) {
  ...
  return NextResponse.json(
    {
      error:
        "Worker profile যাচাই করা যায়নি।",
    },
    { status: 500 }
  );
}
